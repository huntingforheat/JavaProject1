...
public class I01_ipNetAddress {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 127 이상은 음수로 표현됨
			for(byte b : ipAddr) {
				System.out.print((b < 0 ? b + 256 : b)+ "."); // 표현할떄 음수값을 양수화 해줘야 함. b가 0보다 작은 b+256 크면 b 출력
//				System.out.print(b + "."); // 음수값을 양수화 하지 않았을때
			}
			System.out.println();
			
			// 3. getLocalHost 메서드를 이용한 InetAddress 객체 생성 호출
			InetAddress local = InetAddress.getLocalHost();  // localhost 내 컴퓨터
