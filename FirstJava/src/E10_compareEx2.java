...
public class E10_compareEx2 {
	...
	public static void main(String[] args) {
		...
		TreeSet<GymMember> gym2 = new TreeSet<>(new Comparator<GymMember>() {
			...
			public int compare(GymMember o1, GymMember o2) {
				...
				if (o1.name.charAt(0) < o2.name.charAt(0)) {
					...
					return 0; // Set인 경우에는 0이 리턴되면 중복으로 간주 - 값이 안들어감
				}
			}
		});
		...
		gym2.add(new GymMember("봉준", 1003));
		gym2.add(new GymMember("조현우", 1004));
		gym2.add(new GymMember("BTS", 1005));
		gym2.add(new GymMember("박찬호", 1006)); // 인덱스 0번값 박지성이랑 박이 겹쳐서 값이 빠지게됨
...
class GymMember implements Comparable<GymMember>{
	...
	@Override
	public int compareTo(GymMember o) { // 내림차순임 
		// compareTo 메서드는 매개변수 o를 통해서 비교 대상을 전달 받고,
		// - 현재 인스턴스와 o의 자리를 바꾸고 싶은 경우 1을 리턴합니다. 		  비교하는 값보다 내가 더 클떄 자리 교체가 일어남
		// - 현재 인스턴스와 o의 자리를 그대로 유지하고 싶을 때는 -1을 리턴한다. 비교하는 값보다 내가 더 작을떄 자리 교체가 안일어남
		// - 현재 인스턴스와 o가 같은 값임을 나타내고 싶다면 0을 리턴
		int next = o.memNum;
		System.out.println(memNum + " vs " + next);
		if(memNum < next) {
			return -1;   // 값을 1
		}else if (memNum > next) {   // 로 바꿔주면 오름차순으로 바꿀수 있음.
			return 1;	 // 값을 -1
		}else {	
			return 0;
		}
