...
class HttpThread extends Thread {
	...
	public HttpThread(Socket client) {
		this.client = client;
		try {
			br = new BufferedReader(new InputStreamReader(client.getInputStream()));	// 클라이언트가 보낸 데이터 처리를 위해서
	...
	public void run() {
		...
		try {
			String line = br.readLine(); // br의 첫번째 라인을 읽음
			...
			if(fileName.equals("")) {
				fileName = "index.html"; // 대상이 없으면 index.html을 보냄
			}
