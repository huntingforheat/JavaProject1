...
public class B09_ArrayExample1 {
	...
	public static void main(String[] args) {
		...
		String[] welcome = new String[] { "오늘 뭐 드셨어요?", // 인덱스 0번.
				"오늘 저녁에 비온데요. 근데 우산이 없어요.", // 인덱스 1번.
				"졸리죠?" // 인덱스 2번.
		};

		// 배열의 인덱스값 : 배열에 저장된 값을 불러올 떄에 인덱스 값을 사용.
		// 인덱스 값은 시작값이 0부터.
		...
		// 문자열 배열
		char[] s; // 배열 선언.
//		char[5] s2;  // 배열 선언시 오류.[5] 숫자의 의미는 배열의 길이로 생각하고 쓸수 없다.
		// 배열 선언시 크기를 명시 하지 못함.
		...
		/*
		 * (형식) for (타입 변수명: 배열명) { ** 배열명으로 불러오는 배열의 타입은 : 이전의 타입과 같아야함. 반복시 실행할 명령문. }
		 */
		System.out.println("\n강화된 for를 이용한 반복 처리.");
		...
		// 1. 얕은 복사는 byeMessaage 변수에 byeMessage2의 주소값을 대입. 원본값이 달라짐.
//			byeMessage = byeMessage2;
//			byeMessage[1] = "hello bye bye";
//			System.out.println(Arrays.toString(byeMessage));  // 0o는 8진수, 0b는 2진수, 0x는 16진수.
//			System.out.println(Arrays.toString(byeMessage2));
		...
		// length : 복사를 몇개나 할 것인지 결정하는것.

		System.out.println("복사 전 : " + Arrays.toString(byeMessage));
		System.arraycopy(byeMessage2, 1, byeMessage, 5, 2); // byeMessage2에서 1 인덱스 값을 byeMessage에서 5번쨰 자리에 2개를 넣
