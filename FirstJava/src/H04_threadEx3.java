...
public class H04_threadEx3 {
	...
	/*
	 * 	[스레드 동기화]
	 * 		여러 스레드가 동작하는 프로그램에서 하나의 객체를 함께 사용하는 경우, 의도치 않은 결과를 가져올 수 있습니다.
	 * 		하나의 스레드가 작업이 끝날때까지 객체가 변경되지 않도록 하는 것을 동기화(synchronized)
	 * 		
	 ...
	 * 		 - wait() - 멈췄다가 다른 스레드한테 작업을 넘겨줌
	 * 		 - notify() - 실행
	 * 		 - notifyAll() - 멈췄던 스레드 다 실행
	 */
...
class SmartPhoneGame {
	...
	// 메서드에 동기화 설정   특정 자원이 점유하고 있으면 다른 자원은 들어올 수 없는것을 동기화라고 함.
//	public synchronized void increaseLevel() {
	public void increaseLevel() {
		synchronized (this) {		// 2. 동기화 블럭 this는 스마트폰게임 객체를 가르킴
			while(true) {
				this.level++;
				try {
					Thread.sleep(1000);
...
//				if(this.level % 10 == 0) break;
				if(this.level == 5) {    // 레벨5가 플레이어1이 되면 모두 다 실행 
					notifyAll();	// 현재 대기중인 모든 스레드를 실행 상태로 전환
					try {
						// notify() : 현재 대기중인 스레드를 한개만 실행 중으로 변환(직접 지정 불가함)
