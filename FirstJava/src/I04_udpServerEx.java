...
//			socket.setBroadcast(true); // 브로드캐스트 설정
			...
			while(true) {
				// 데이터를 받기위한 바이트배열 생성
				inMsg = new byte[1024];  // 1024 또는 512바이트로 생성
				// DatagramPacket 객체 생성
				inPacket  = new DatagramPacket(inMsg, inMsg.length);
				// 패킷데이터 수신
				socket.receive(inPacket);
				// 문자열로 저장
				String msg = new String(inMsg, 0, inPacket.getLength()); // 실제로 받는 데이터는 1024보다 적을수 있기 때문에 패킷에서 받는 길이만큼으로 설정
