...
public class A04_varTypes {
	...
	public static void main(String[] args) {
		 ...
		 * 정수형 : 양의정수, 음의정수, 0
		 * 	- byte(1byte)
		 * 	- char(2byte)	* 주의 - 한글자의 크
		 * 	- short(2byte)
		 * 	- int(4byte)	*정수형 기본 타입*
		 * 	- long(8byte) - 큰 정수값을 표기하기 위해
		 * 
		 * 	- 1byte (byte, char(c언어) )
		 * 	  signed : -128 ~ 127
		 *	  unsigned : 0 ~ 255
		 ...
		 * 	- 8byte (long) -
		 * 	  signed : -2^63 ~ 2^63-1, unsigned : 
		 */
			
		byte _byte = -128;
		short _short = 10;
		char _char = 65535;
		int _int = 2_100_000_000; // 콤마대신 _바로 자릿수 표
		long _long = 22_0000_0000L; // long은 정수 뒤에 L(l)을 붙여 사
		 ...
		 * 참조형 (클래스 타입)
		 * 	String (문자열)
		 *	그외 모든 타입 대문자로 시작하는 모든 타
		 */
		
		// 데이터 타입이 소문자로 시작하는 타입 : 기본 타입
		// 데이터 타입이 대문자로 시작하는 타입 : 참조 타입
		String comment = "앞으로 약 35분후 점심시간입니다.";
