...
public class D10_Calendar {
	...
	public static void main(String[] args) {
		// Calendar는 생성자를 사용할 수 없고, genInstance() 메서드를 이용하여 인스턴스를 생성한다.
		// 인스턴스 생성한 시점의 시간 정보를 생성.
		Calendar cal = Calendar.getInstance();
		
		System.out.println(cal); 
		
		// Calendar의 get() 메서드와 Calendar 상수를 이용헤 원하는 정보를 가져올 수 있음.
		int year = cal.get(Calendar.YEAR);
		int month = cal.get(Calendar.MONTH) + 1; // month는 첫달이 0부터 시작하므로 +1을 해줌
