...
public class Quiz_shop_240315_answer {
...
class Buyer {
	...
	void buy(Product p) {
		...
		if(cash < p.getPrice()) {
			...
			return;
		}else if(t_index == MAX_TRUNK_SIZE) { // 이 경우 물건 하나를 더 채울려고 하는 상황
	...
	int getNullCount() { // 트렁크에 들어갈 카트 공간 체크
		int count = 0;
		for(int i = 0; i < c_index; i++) {
			if(cart[i] == null) count ++; // 카트의 공간이 null이면 공간++
		}
		return count;
	}
	
	private void clearCart() { // 구매를 하고 나서는 카트를 비워야되니 새로운 카트 만들기(카트 비우기)
		cart = new Product[INIT_CART_SIZE];
		c_index = 0; // cart를 다시 초기화
	}
	...
	void addToCart(Product p) {
		// 카트에 비어 있는 공간이 있는 경우
		for(int i = 0; i <c_index; i++) {
			if(cart[i] == null) { // 카트에 null값이 있는 경우
				cart[i] = p;
		...
		if(c_index == cart.length) { 
			...
			System.arraycopy(cart, 0, newCart, 0, cart.length); // 다시 보기
			cart = newCart;
		}
...
class Coffee extends Product {
	...
	void drink() {
		System.out.println("'" + name + "' 커피를 끓여 마셨씁니다.");
	}
}
