...
public class D15_stringEx {
	...
	public static void main(String[] args) {
		...
		System.out.println(str.substring(0, 5)); // 5번 인덱스 포함 안됨
		System.out.println(str.substring(5));
		
		System.out.println(str.substring(7, 12));

		// 2. split : 문자열을 원하는 기준으로 자른다. -> 배열로 변환
		...
		System.out.println("두번쨰 o의 우치 : " + str.indexOf('o', str.indexOf('o')+1));
		...
		str = str.replace("naver", "gmail"); // naver를 gmail로 치환
		System.out.println(str);
		str = "abc@naver.com";
		str = str.replaceFirst("naver", "korea"); // naver라는 문자를 찾아서 korea로 변경
		System.out.println(str);
		
		// 8. lastIndexOf : 해당 문자열을 뒤에서부터 찾아서 위치를 변환
		str = "abcd    dcba";
		...
		System.out.println("after1 : " + id_1.trim());
		System.out.println("after2 : " + id_2.trim());
		
		// 11. format : 서식문자열을 이용하여 문자열을 만들어서 반환
		str = String.format("%d:%d:%d", 11, 14, 50);
		System.out.println(str);
		
		// 12. endsWith : 지정된 문자로 끝나느지 검사 (확장자 체크할 때 사용)
		str = "오늘할일.html";
