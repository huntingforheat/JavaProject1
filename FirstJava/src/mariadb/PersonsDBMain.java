...
public class PersonsDBMain {
	...
	static void insert() {
		...
		if(result != 0) System.out.println("레코드 추가 성공");
	...
	static void update() {
		...
		System.out.print("수정할 레코드 id를 입력해 주세요 : ");
		vo = dao.selectOne(scan.nextInt());
		
		System.out.print("lastName("+vo.getLastname()+") 수정 : ");	// 기본정보를 노출시켜주면서 수정을 할 수 있음
		vo.setLastname(scan.next());
		...
		if(result != 0) System.out.println("레코드 수정 성공");
