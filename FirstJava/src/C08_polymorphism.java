 ...
 * 	  (다형성 인자 예제)
 * 	 -  다형성을 적용하는 방법은 부모클래스의 객체를 선언하고, 자식클래스의 객체를 할당하는 것. 반대의 경우에는 불가능함.
 *   -  다형적 객체의 경우 부모 클래스에 있는 메서드만 사용이 가능함.
 *     자식 클래스에 재정의 되어 있다면 실행은 자식클래스에 있는 메서드로 실행됩니다.
 *     자식클래스의 메서드가 실행된다고해서 부모클래스에 있는 해당 메서드를 지우면 안됩니다.
...
public class C08_polymorphism {
	...
	public static void main(String[] args) {
		...
		p.walk(); // 상속받은 메서드 실행
//		p.study(); // p는 다형적 객체이기 떄문에 부모 클래스에 있는 메서드만 사용가능.(study는 자식클래스(student)에 있기 때문에 사용불가.
		Student casting = (Student)p; // 캐스팅하면 자식클래스에 있는 메서드도 사용 가능
		// study() // 사용해야 하는 경우에는 Student로 캐스팅이 되어야 사용이 가능
		casting.study();     
		
		// 예제
		Person p2;
		p2 = new Teacher("홍선생", 22, "Java Programing");
		// instanceof는 연산자는 좌항의 객체와 우항의 클래스를 비교하는것 (true/false 값으로 변환)
