 ...
 * 
 *  ## static의 의미.
 *    - 일반적으로 각 인스턴스들은 서로 독립적으로 객체 내에서 변수값을 서로 각각 다름.
 *    - Person p1, Person p2가 있는 경우에 각각의 p1.getName(), p2.getName()는 다른 값을 가질 수 있음.
 *    - static이 변수 앞에 붙으면 모든 인스턴스에 대해서 공유가 됩니다. 이때 변수는 인스턴스 변수가 아닌 클래스 단위로 바뀌게 되고, 이 때문에 static이
 *    붙은 멤버 변수를 클래스 변수라고 합니다. 그리고, static이 붙지 않은 변수를 인스턴스 변수라고 하며 구분합니다.
 *    (static)이 붙으면 - 모든 인스턴스에 대해 공유가 가능, static이 붙은 멤버 변수를 클래스 변수라고 함.
 *    (static)이 안 붙으면 - 인스턴스 변수라고 함.
 *    - 메서드 static이 붙는다면 해당 메서드는 인스턴스와 무관하기 때문에 인스턴스를 생성하지 않아도 호출이 가능함.
 *    
 *    -- static 관련 된 자바 문법
 *     > static 메서드 안에서는 non-static 멤버를 객체 생성 없이 직접 참조할 수 없음.
 *     > 반대로 static이 붙지 않은 메서드에서는 static 멤버를 사용할 수 있음.
 *     
 *    - 주로 모든 인스턴스가 공통된 변수값을 가져야 하는 경우가 있는데 이떄 static 변수를 사용합니다.
...
public class C06_static {
	...
	public static void main(String[] args) {
		...
		System.out.println("static b : " + c2.b); // b값이 다른이유는 static이어서 공통된 자원이기 떄문에 위에서 1이여서 ++되서 2가 나옴.
