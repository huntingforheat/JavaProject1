 ...
 *    - Object 클래스를 최상위 클래스
 *    - final 클래스는 상속 불가. 말 그대로 마지막이라는 말이므로 상속을 받을순 있지만 줄순 없음
...
public class Person {
	...
	public String name;
	public int age;
	protected float height; // protected는 상속받을떄 사용가
	float weight;
	...
	public Person(String name, int age, float height, float weight) { // source - fields 생성자를 이용해서 초기화
		this.name = name;
		this.age = age;
		this.height = height;
		this.weight = weight;
	...
	public boolean equals(Object obj) {
		...
		Person other = (Person)obj; // 부모는 상속이 안되므로 캐스팅 해주어서 같게 만들어줌
		if(age != other.age) return false; // 객체 내에 age멤버의 값을 비교 확인
		if(name == null) {
			if(other.name != null) {
				return false; // 객체 내의 이름 확인
			}
		}else if(!name.equals(other.name)) return false; // equals서 나이와 이름을 비교함
