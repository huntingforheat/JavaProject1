...
//	}
	...
	/*
	 * ## 생성자.
	 * - 생성자는 반드시 클래스 ㅇ나에 하나 이상 선언해야 함.
	 * 	선언하지 않으면, JVM(JAVA VIRTURAL MACHINE)이 대신해서 기본 생성자를 자동으로 만들어 줌.
	...
	// 생성자 생성 예시.
	public Pen() { // 가로안에 아무것도 없으면 기본 생성
		color = "black";
	}
	...
	public Pen(String color, int length, int price) {
		this.color = color;  // 위에 이름이 같으므로 매개변수와 멤버변수를 구분하기 위해 this을 사용해줌.
		this.length = length;
		this.price = price;
	}
	
	// 오버로딩으로 구분되는 생성자 혹은 메서드는 매개변수의 숫자와 타입에 의해서 결정됨.
	// 조건
	...
	// 4. 매개변수의 개수가 달라야 한다.
	// 5. 매개변수의 순서가 달라야 합니다.
	public Pen(int length, String color) { // 위에 같은 생성자가 있어도 데이터 타입 순서를 바꿔주면 중복되지 않음.
		this.color = color; 
		this.length = length;
	}
