...
public class G16_optionalEx {
	...
	/*
	 * 	집계 메서드
	 * 		스트림 API에서 요소들의 최소값, 최대값, 합계, 평균값 갯수 등을 구할 수 있는 메서드를 제공 하고 있음.
	 * 		합계와 갯수를 구해주는 메서드 이외 Optional로 시작하는 타입으로 리턴
	 */
	...
	public static void main(String[] args) {
		...
		// 요소값이 있는 경우에만 출력
		if(avg2.isPresent()) { // 값의 존재여부 확인
		System.out.println("요소들의 평균 : " + avg2.getAsDouble());
		}
		
		// orElse() : 기본값 설정
		int max2 = list2.stream().mapToInt(Integer::intValue).max().orElse(0); // 값이 없으면 0 출력
		...
		// 	reduce메서드는 매개변수 타입이 XXXOperator를 이용하고, Stream, IntStream, LongStream, DoubleStream을 전달
		...
		double areaSum = list.stream().mapToDouble(Shape1::area).sum();
		System.out.println("sum()을 이용한 면적 합계 : " + areaSum);
		
		areaSum = list.stream().mapToDouble(Shape1::area)
