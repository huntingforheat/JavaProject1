...
public class G17_collect_groupBy {
	...
	/*
	 * 	collect() 메서드는 스트림의 요소들을 필터링하거나 매핑한 후에 새로운 객체로 생성
	...
	public static void main(String[] args) {
		...
		List<Shape1> rectList = list.stream()
				.filter(s -> s instanceof Rectangle1).collect(Collectors.toList());
		rectList.stream().forEach(f -> System.out.println(f));
		System.out.println();
		
		// 요소가 Rectangle 객체인 경우 collect 메서드로 set으로 변환
		Set<Shape1> rectSet = list.stream()
				.filter(s -> s instanceof Rectangle1).collect(Collectors.toSet());
		rectSet.stream().forEach(System.out::println);
		
		// groupingBy() Map 객체 생성
		try {
			// 객체 타입으로 그룹핑 (Rectangle, Circle)
			Map<Object, List<Shape1>> map = list.stream()
					.collect(Collectors.groupingBy(f -> f.getClass()));
			System.out.println("사각형 출력");
			map.get(Class.forName("Rectangle1")).stream()
					.forEach(f -> System.out.println(f));
			
			map.get(Class.forName("Circle1")).stream()
				.forEach(f -> System.out.println(f));
		} catch (Exception e) {
